...
void ObtenirTourIA(Point &pionPos, Point &attaquePos, bool tour) {
	...
	// Obtention de la liste des pions les plus avancés jouables
	std::vector<Point> pions;
	int rang = (tour) ? INT_MAX : INT_MIN;
	for(int y = 0; y < hauteur; y++) {
		for(int x = 0; x < longueur; x++) {
			if(tableau[y][x] == monPion && PionJouable({x, y}, tour)) {
				bool condition = (tour) ? (y < rang) : (y > rang);
				if(condition) {
					pions.clear();
					rang = y;
				}
	...
	// Obtentions des diverses attaques possibles en séparant les attaques qui avancent de celles qui mangent
	int nombrePions = pions.size();
	std::vector<std::vector<Point>> attaquesAvance;
	std::vector<std::vector<Point>> attaquesMange;

	int nombreTotalAttaquesAvance = 0;
	int nombreTotalAttaquesMange = 0;
	int valTmp = -1;
	for(int i = 0; i < nombrePions; i++) {
		Point pionActuel = pions[i];
		std::vector<Point> atqA;
		std::vector<Point> atqM;
		...
		for(int y = 0; y < hauteur; y++) {
			for(int x = 0; x < longueur; x++) {
				if(CoupValide(pionActuel, {x, y}, tour)) {
					if(pionActuel.x == x) {
						atqA.push_back({x, y});
						nombreTotalAttaquesAvance++;
					}
					else {
						atqM.push_back({x, y});
						nombreTotalAttaquesMange++;
					}
				}
			}
		}

		attaquesAvance.push_back(atqA);
		attaquesMange.push_back(atqM);
	}

	// Génération du nombre aléatoire correspondant au numéro d'attaque
	int nbAttaque = (nombreTotalAttaquesMange == 0) ? nombreTotalAttaquesAvance : nombreTotalAttaquesMange;
	int rnd = rand() % nbAttaque;

	// Itération à travers les attaques jusqu'à arriver à celle correspondant au numéro généré, pour définir pionPos et attaquePos
	bool casser = false;
	int numeroActuel = 0;
	for(int i = 0; i < nombrePions; i++) {
		std::vector<Point> attaques = (nombreTotalAttaquesMange == 0) ? attaquesAvance[i] : attaquesMange[i];
		int nombreAttaques = attaques.size();

		for(int j = 0; j < nombreAttaques; j++) {
			if(numeroActuel == rnd) {
				pionPos = pions[i];
				attaquePos = attaques[j];
				casser = true;
				break;
			}
